...
Read with the [Design Doc](/docs/designs/current_design_doc.md). Where this
document and that one disagree, the code wins — see §6.

Generated against v0.3.2 (`f59655a`). Every claim cites
`path — symbol(), lines N–M`, read from the tree at that commit.

**Changed since the v0.3.1 edition.** One stop moved: §2.8, where
`cmd_validate()`'s per-concept loop no longer `continue`s past the whole body
for the bundle's root `index.md` and `log.md`. Everything below that line in the
function shifted down three lines, so most of the citation changes in this
document are arithmetic rather than substance. §4 gains one case (24 → 25) and
§6.4 loses its longest-standing complaint: the curate shell test is finally on a
gate.
...
model *when* to do graph work; one 989-line Python script does it
...
|---|---|---|
| `.claude-plugin/plugin.json` | Manifest: name `okf-graph-eng`, version `0.3.2` | The host's entry point |
...
| `scripts/okf-graph.py` | **The graph engine**, 989 lines | Everything deterministic |
...
| `tests/test_okf_graph.py` | 25 plain-assert cases | Regression net for the engine |
| `tests/test_okf_curate.sh` | 5 bash checks, added v0.3.1 | Regression net for the curation hook. On CI and pre-commit since v0.3.2 — see §6.4 |
...
```mermaid
flowchart TD
    MAIN["main() — argparse, 8 subparsers<br/>lines 916–985"]
    subgraph CMD["Subcommands"]
        direction LR
        I["cmd_impact 441"]; B["cmd_backlinks 484"]; S["cmd_subgraph 505"]; P["cmd_pack 548"]
        E["cmd_edges 651"]; G["cmd_graph 747"]; V["cmd_validate 820"]; O["cmd_orphans 903"]
    end
...
## 2.8 `cmd_validate()` — lines 820–900, and the `--strict` contract

Eight rules across three severities (the full table is in the design doc, §13).
Two parts are worth stopping on: the top of the loop, which v0.3.2 changed, and
the last three lines, which are a contract.

### The top of the loop: what "exempt" means (v0.3.2)
...
```
`scripts/okf-graph.py — cmd_validate(), lines 825–834`

Until v0.3.2 those ten lines were six, and they opened differently:
...
```python
        ...
        if not c.meta.get("title") and c.path.name != "index.md":
            ...
```
`scripts/okf-graph.py at v0.3.1 (c9c6c99) — cmd_validate(), lines 826–831`

The `continue` was written for the metadata exemption. But everything else in
the loop body — broken-link detection, the off-bundle warning, the
non-standard-rel `info`, `TicketLink` hygiene — comes *after* it. So the root
`index.md` and root `log.md` were skipped by all of it. The bundle's entry
point, the file every reader and every skill starts from and the one with the
most inbound links, was the single place in the bundle where a broken link was
never reported.

**Read the fix for what it actually is.** Both old checks already carried
`c.path.name != "index.md"`, so the `continue` was doing nothing for any
`index.md`. And `load_bundle()` gives an `index.md` the type `Index` when its
frontmatter supplies none:

```python
            type=str(meta.get("type") or ("Index" if path.name == "index.md" else "Unknown")),
```
`scripts/okf-graph.py — load_bundle(), line 306`

so the type check could not have fired for it anyway. The only file that
genuinely needed the broad `continue` was the root `log.md`, which has no
frontmatter at all. The `structural` boolean is that one real exemption written
down; the rest of what the `continue` did was accidental.

**Where the width matters.** `c.path.name == "index.md"` is by filename, so it
covers a nested `a/index.md` too — matching the old guard exactly. `rel ==
"log.md"` is root-relative, so a nested `a/log.md` is an ordinary concept and
still owes a type and a title. That asymmetry is deliberate and the test asserts
it (§4.5a). The separate orphan exemption further down (`lines 866–867`) is
untouched: a root index with no inbound link is normal, not an orphan.

The whole change is +9/−6 lines, three of them the comment above. Counting only
executable lines it is a wash — six out, six in.

### The last three lines: the `--strict` contract
...
```
`tests/test_okf_graph.py — lines 400–401`
...
`test_subgraph_neighbourhood_is_symmetric(), lines 372–381` is the interesting
...
| I3 | `validate` exits 0 on warnings unless `--strict` | `cmd_validate():898–900` | Every skill that calls `validate` mid-conversation reads a normal in-progress bundle as a failure |
...
| I8 | The four version manifests agree | `test_version_is_consistent_across_manifests():404–421` | Silent version drift between Claude Code and Grok Build install paths |
...
`tests/test_okf_graph.py` — **25 cases** (24 at v0.3.1, 16 at v0.3.0), plain
`assert`, no framework, no fixtures, no dependencies. Its own docstring bounds
the ambition (lines 6–9): "Kept deliberately small: it exists to catch the
defects that shipped in v0.2.0 … and to stop sample-okf and the four version
manifests from drifting."

v0.3.2 adds exactly one case — §4.5a, the only new test in this release.

The eight cases added in v0.3.1 each pin a behaviour that release changed, and
...
## 4.4 Strict vs lenient — lines 383–402
...
## 4.5 The drift tripwires — lines 207–215 and 404–421
...
value. Neither tests behavior; both catch unreviewed edits. Verified at v0.3.2
(`f59655a`): `validate sample-okf --strict` reports 22 concepts, 83 edges (13
typed), 0 errors, 0 warnings, 0 issues — unchanged since v0.3.0. Worth noting
that the v0.3.2 validator change did not move these numbers either: `sample-okf`
had no broken or off-bundle links hiding in its root files. The blind spot was
real; this bundle simply was not in it.

## 4.5a The root-file blind spot — lines 347–369 (new in v0.3.2)

The one test added in v0.3.2, and the clearest example in the suite of a test
...
**Rule proved:** the structural exemption is scoped to `type` and `title`. Links
in the bundle's root files are validated like anyone else's, and the exit code
goes non-zero for them like anyone else's.

**Catches:** two opposite regressions. Restore the old `continue` and the first
four assertions fail. Over-correct by deleting the exemption entirely — the
tempting one-line "fix" — and the first four assertions still pass while the
last one fails, because the root `index.md` and root `log.md` in this fixture
deliberately carry no title. Note the fixture's shape: none of the four files
has a title, and `sub/log.md` is the only one that should be told about it.

The bundle also proves the depth asymmetry §2.8 describes without a separate
case: `sub/index.md` is exempt (filename match) while `sub/log.md` is not
(root-relative match).
...
**It runs on both gates as of v0.3.2.** For two releases it ran on neither — see
§4.8. Run it by hand the same way the gates do: `bash tests/test_okf_curate.sh`
— silent means passing.
...
# Post-edit hook tests, same guarded pattern.
[ ! -f tests/test_okf_curate.sh ] || bash tests/test_okf_curate.sh >/dev/null 2>&1 || fail "curate tests failing — run: bash tests/test_okf_curate.sh"
```
`hooks/pre-commit — lines 95, 98`

The local hook is bypassable with `--no-verify`; CI is not.

**The second of those lines is new in v0.3.2.** Until then
`tests/test_okf_curate.sh` appeared in neither file: the repository's newest test
protected the component with the worst track record, and nothing ran it. The
shape of that gap is worth remembering, because it is the same shape as the
v0.2.0 defect the test exists to catch — something correctly written, correctly
committed, and never actually invoked. Both are now closed the same way: by
checking that the thing runs, not that it exists.
...
| `validate` is unexpectedly non-zero | `cmd_validate():898–900` — is `--strict` set? Are there real errors, or only warnings? |
...
| Change | Files |
|---|---|
| New subcommand | `scripts/okf-graph.py`: a `cmd_*` function, a subparser in `main()` (916–961), a dispatch line (969–984). Then `docs/user_guide/cli-reference.md`. If it takes a concept, resolve it with `resolve_or_error()` — never `resolve_concept()` directly, or it will not report ambiguity |
...
| `scripts/okf-curate.sh` | Runs on every Write/Edit/MultiEdit in every repository. Its test is on both gates since v0.3.2, so a regression here now fails the commit rather than reaching users |
| `cmd_validate()`'s per-concept loop | Everything in the body applies to every concept except the two metadata checks. A `continue` added at the top silently disables link validation for whatever it skips — that was the v0.3.2 bug (§2.8) |
...
| "No test for `criticality_of()` … none for `scripts/okf-curate.sh`" | Both exist (§2.9a, §4.6), and since v0.3.2 both run on CI and pre-commit |
...
**And where the v0.3.1 edition went stale.** Two claims, both closed by v0.3.2:

| v0.3.1 claim | Reality at v0.3.2 |
|---|---|
| "`index.md` and `log.md` are exempt from the per-concept checks" (design doc §13) | Exempt from the *type and title* checks only. Their links, rels and TicketLink hygiene are validated like every other concept (§2.8) |
| "`tests/test_okf_curate.sh` … runs on no gate" (§4.6, §6.4, R10) | On `hooks/pre-commit:98` and the CI step "curate hook tests" (§4.8) |

The first of those was not merely a documentation error — the document
accurately described what the code did. The code was wrong, and describing it
precisely is what made that visible.
...
**The gap this section led with for two editions is closed.**
`tests/test_okf_curate.sh` was referenced by neither
`.github/workflows/worklog.yml` nor `hooks/pre-commit` — a test on no gate
protects nothing, structurally the same defect as the v0.2.0 hook that was
configured but never fired. v0.3.2 added one line to each file (§4.8).

**What is left.** Four of eight subcommands — `impact`, `backlinks`, `edges`,
`orphans` — have no direct output-*shape* test. `impact`, `backlinks`,
`subgraph` and `pack` now have their ambiguity failure path covered
(lines 303–321), and `subgraph`'s traversal property is covered (lines 372–381),
but nothing asserts the payload keys the skills actually parse. There is still
no test for `scripts/okf-ticket-link.py`.

**Worth noticing about the v0.3.2 fix.** `cmd_validate()` had the best test
coverage of any subcommand — three cases before this release — and the bug was
still there, because every one of those tests supplied a fixture in which the
root files had nothing wrong with them. Coverage of a function is not coverage of
its inputs. The gap was closed by widening the fixture, not by adding assertions
to the existing ones.

**Closed since v0.3.0:** `criticality_of()` and `enrich_nodes()` ordering
(lines 121–144), and `scripts/okf-curate.sh` (§4.6) — the latter now gated.
...
Everything in §§1–5 and §6.0–6.5 was read directly from the tree at `f59655a`.
The counts (22 concepts, 83 edges, 13 typed, 0 errors, 0 warnings; 25 graph tests
...
`bash tests/test_okf_curate.sh` against that commit. Quoted v0.3.1 code in §2.8
is from `c9c6c99` and quoted v0.3.0 code is from `6bf5d65`; both are marked as
such at each quotation.

**The inference the previous edition carried is now an observation.** It flagged
one prediction — that a test on no gate would eventually fail to catch a
regression. v0.3.2 gated the test before that prediction could be tested, so it
remains untested and is now moot.

**The one inference remaining in this document** is the claim in §2.8 that the
root `index.md` blind spot was the *most* consequential place for it to be,
because that file has the most inbound links. That is true of `sample-okf` and
of every bundle the repository's own generators emit; it is a strong convention,
not something the code enforces. What is confirmed is only that before v0.3.2 no
check in the loop body ran for the root `index.md` or `log.md`.
