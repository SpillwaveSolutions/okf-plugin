---
wiki_key: design/2026-08-02_v0.3.1-release-design-doc
doc_type: design
truth_state: snapshot
title: Design Doc 2026-08-02 v0.3.1-release
date: 2026-08-02
name: v0.3.1-release
tag: v0.3.1
...
> Generated against the v0.3.1 tag. Every code claim below cites
> `path — symbol(), lines N–M` and was read from the tree at
> `c9c6c99`. Claims are labelled **Confirmed** (read in the code),
> **Assumption**, **Recommendation**, or **Open Question**.
>
> **What changed since the v0.3.0 edition.** v0.3.1 is a correctness release:
> it fixed logic that was dead, silent, or wrong without ever crashing. Three
> of that document's own open items are now closed in code — the dead
> `criticality_of` branch (was Q2), ambiguous concept resolution (was Q3), and
> the curation hook's hard-coded path filter (was R4). §9.2, §13, §8.4 and §20
> carry the new behaviour.
...
- [Code Walkthrough](/docs/designs/2026-08-02_v0.3.1-release_code_walkthrough.md) — the guided tour of the same code
...
| Graph engine (`scripts/okf-graph.py`, 986 lines) | Deterministic CLI: eight subcommands over a bundle |
...
| `tests/test_okf_graph.py` | 24 plain-assert cases over the engine |
| `tests/test_okf_curate.sh` | 5 checks over the curation hook — the repo's first shell test (new in v0.3.1) |
...
The new shell test for the curation hook is not wired into any gate (§20 R10).
...
| F7 | Validate structure, links, and graph hygiene | `cmd_validate(), lines 820–897` |
...
| N6 | The plugin version cannot drift across its four manifests | `test_version_is_consistent_across_manifests(), lines 379–396` |
| N7 | Skills must keep working when the validator emits warnings | `cmd_validate()` returns 0 on warnings unless `--strict`, lines 895–897 |
...
`cmd_validate():821`, `cmd_orphans():901`). There is no cache and no incremental
...
dangling target as a broken link (`cmd_validate():838–840`). Conflating the two
...
`link outside bundle → …` at severity **warn** (lines 832–837). The prior
...
non-zero (`cmd_validate(), lines 895–897`).
...
**Pinned by:** `test_strict_validate_flags_warnings(), lines 358–377`.
...
above the check says exactly this (lines 832–833).
...
| `scripts/okf-graph.py` | Python CLI, 986 lines | All deterministic graph operations | Bundle path, concept path, flags | JSON / Mermaid / HTML | stdlib only | Every skill degrades to manual file crawling |
...
| `tests/test_okf_graph.py` | 24 plain-assert cases | Regression net for the engine | — | Exit code | `okf-graph.py`, `sample-okf`, 4 manifests | Regressions ship silently (the v0.2.0 condition) |
| `tests/test_okf_curate.sh` | 5 bash checks | Regression net for the curation hook | — | Exit code | `okf-curate.sh`, `okf-graph.py` | Hook defects ship silently again — and it is on no gate today (R10) |
...
"non-standard rel … (allowed but uncommon)" (lines 841–848). The vocabulary is
advisory by design.
...
1. New subcommand — add a `cmd_*` function plus a subparser in `main()`
   (lines 913–982) and a dispatch line (966–981). No other file changes.
...
`test_subgraph_neighbourhood_is_symmetric(), lines 347–356` locks the property
...
**Validation rules** (`cmd_validate(), lines 820–897`):

| Severity | Condition | Line |
|---|---|---|
| error | bundle has no root `index.md` | 823–824 |
| error | edge target is not a loaded concept (broken link) | 838–840 |
| warn | non-index concept with type `""` or `Unknown` | 828–829 |
| warn | non-index concept with no `title` in frontmatter | 830–831 |
| warn | **link target resolves outside the bundle** (new in v0.3.1) | 832–837 |
| warn | `TicketLink` with neither `external_id` nor `worklog_id` | 850–858 |
| warn | concept in `HIGH_IMPACT_TYPES` with `verified: false` | 870–878 |
| info | frontmatter relation outside `KNOWN_RELS` | 841–848 |
| info | orphan — no inbound and no outbound links | 859–869 |

`index.md` and `log.md` are exempt from the per-concept checks (line 826), and
...
(line 897). The comment above it states the constraint explicitly: "the skills
...
comment says so (lines 832–833): "the default exit code must stay 0 for the
...
| Broken link | Reported as a validation error; excluded from traversal | `load_bundle():319–320`, `cmd_validate():838–840` |
...
| Link target outside the bundle | Not an edge; reported by `validate` as a warning | `_normalize_target():225–228`, `cmd_validate():832–837` |
...
| CLI over `sample-okf` | `sample_bundle_validates`, `pack_mermaid_has_no_collapsed_nodes`, `graph_mermaid_is_a_fenced_block`, `graph_json_shape`, `graph_focus_narrows_the_node_set`, `graph_focus_unknown_concept_errors`, `graph_html_is_self_contained` (207–301), **`subgraph_neighbourhood_is_symmetric` (347–356)** | Subprocess, real bundle |
| CLI over a temp bundle | **`ambiguous_concept_is_a_cli_error` (303–321), `validate_reports_off_bundle_links` (323–345)**, `strict_validate_flags_warnings` (358–377) | Subprocess, synthetic bundle |
| Repo invariants | `version_is_consistent_across_manifests` (379–396) | File reads |

**Bold rows are new in v0.3.1.** Every one of the eight added cases pins a
behaviour this release changed, and five of them were written as regressions
against a defect that shipped: the decorative medium tier, the guessed
ambiguous match, the vanished off-bundle link, the dot-directory walk, and the
two-pass adjacency.
...
Verified against the tree at v0.3.1 (`c9c6c99`): validate reports exactly
22 / 83, 0 errors, 0 warnings, 0 issues. Of the 83 edges, 13 are typed (`edges`
subcommand). Unchanged from v0.3.0 — no v0.3.1 change touched `sample-okf`.
...
| *(none)* | `bash tests/test_okf_curate.sh` | — it runs nowhere automatically |
...
**Confirmed gap.** `tests/test_okf_curate.sh` is not referenced by
`.github/workflows/worklog.yml` or by `hooks/pre-commit`. It exists, it passes,
and nothing runs it — which is the same shape of defect as the v0.2.0 hook that
was wired up but never fired. See §20 R10.
...
- `tests/test_okf_curate.sh` is on no gate (§20 R10).

**Closed since v0.3.0:** `criticality_of()` and `enrich_nodes()` ordering are
now covered (lines 121–144), and `scripts/okf-curate.sh` has a test — the
v0.2.0 hook defect, and the v0.3.0 path-filter defect, would both be caught by
it today if it were wired to a gate.
...
# curation hook tests (silent on success; on no gate — run it by hand)
...
| R10 | `tests/test_okf_curate.sh` runs on no gate — not CI, not pre-commit | `.github/workflows/worklog.yml`, `hooks/pre-commit` | high | medium | None today; it must be run by hand | Immediately — a test nothing runs is the defect it was written to prevent |
...
|---|---|---|---|
| 1 | Wire `bash tests/test_okf_curate.sh` into CI and `hooks/pre-commit` | Closes R10. One line in each file; the test already exists and passes | — |
| 2 | Output-shape tests for `impact`, `backlinks`, `edges`, `orphans` | Closes R2; these are the payloads the skills parse | — |
| 3 | Extract one `undirected_adjacency()` helper | Closes R6; three identical blocks remain in subgraph / pack / graph | — |
| 4 | Optional PyYAML path with the hand parser as fallback | Closes R1 without adding a hard dependency | Evidence that users are hitting the edges |
| 5 | Cached-fixture mode for `substack_okf.py` | Makes the end-to-end harness CI-safe | — |
| 6 | Real `docs/adr/` records, or drop the empty directory | Closes R9 | Decision on where ADRs live |

**Done in v0.3.1**, from the v0.3.0 edition of this list: item 2 (the
`criticality_of()` medium branch — decided and fixed, §9.2), item 3 (the
`okf-curate.sh` path filter — replaced rather than widened, §8.4), and the
redundant half of item 4 (`cmd_subgraph()`'s second adjacency pass, §11).
...
point of silent failure for user-authored bundles — R1. (2) The repository's
first shell test runs on no gate, so the component it covers is exactly as
unprotected as it was before the test was written — R10. (3) Four of eight
...
**Recommended implementation order.** §21, item 1 first — it is two lines and
converts an existing test into an actual gate. Then items 2–3; all are additive
and none changes a shipped output shape.
...
and the same is true of every defect v0.3.1 fixed.
...
that stood in for a test it did not perform. None of these produced a stack
trace. All of them were found by reading the code, which is the only method that
finds them. For a tool whose output is an assertion about what depends on what,
being quietly wrong is the failure mode that matters most.
