...
never escalates no matter its state. (open)
...
whichever match comes first in iteration order. (open)
...
so the two clauses before it can never decide anything. (open)
...
closed, and points at the pre-move repository. (open)
...
harmless only because the result is deduped immediately after. (open)
...
collided with this repo's pull request numbers. (open)
...
sees it, so a mistyped path is invisible rather than broken. (open)
...
rooted anywhere else is skipped entirely. (open)
...
repo root pulls the work log and other internal trees in as concepts. (open)
